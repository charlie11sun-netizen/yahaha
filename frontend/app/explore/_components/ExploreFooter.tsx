    ...
    <footer className="border-t border-slate-200/80 bg-white/70 backdrop-blur">
        ...
        {footerColumns.map((column) => (
          <div key={column.title} className="flex flex-col gap-2.5">
            ...
            {column.links.map((link) => (
              <Link key={link} href={footerRoutes[link] || "/about"} className="text-left text-xs text-slate-500 transition-colors hover:text-violet-600">
                {link}
              </Link>
            ))}
          </div>
      ...
      <div className={cn(SHELL, "flex flex-col items-center justify-between gap-3 border-t border-slate-200/70 py-5 sm:flex-row")}>
        ...
        <div className="flex gap-5">
          <Link href="/privacy" className="text-xs text-slate-400 transition-colors hover:text-violet-600">Privacy Policy</Link>
          <Link href="/terms" className="text-xs text-slate-400 transition-colors hover:text-violet-600">Terms of Service</Link>
          <Link href="/about" className="text-xs text-slate-400 transition-colors hover:text-violet-600">Docs</Link>
        </div>
      </div>
    </footer>
  );
