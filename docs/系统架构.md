...
| # | 节点 | 调模型 | 往 state 追加（太空射击示例） |
| --- | --- | --- | --- |
| 1 | `safety_intake` ◆ | 否 | `normalized_prompt`、`safety_result`{passed:true} |
| 2 | `intent_spec` | 是 | `game_spec`{title:Space Fortress, genre:shooter, theme:space} |
| 3 | `brief_expansion` | 是 | `expanded_brief`{core_verbs:[fly,dodge,shoot], min_content} |
| 4 | `mechanic_planner` | 是 | `mechanic_plan`{enemies:[swarm,gunship,boss], powerups×3} |
| 5 | `archetype_router` | 否 | `archetype_result`=vertical_shooter（写回 `game_spec.archetype`） |
| 6 | `asset_processing` | 否 | `asset_manifest`{cover:渐变, assets:[]} |
| 7 | `game_design` | 是 | `game_design`{900×600, entities, waves, boss:mothership} |
| 8 | `content_plan` | 否 | `content_plan`{tutorial, waves[4], hazard/reward 名} |
| 9 | `balance_plan` | 否 | `balance_config`{65s · target170 · lives4 · spawn1750ms} |
| 10 | `code_generation` | 是 | `generated_files`[index.html · style.css · game.js] |
| 11 | `build_validation` ◆ | 否 | `validation_result`{valid, 每文件 sha256/size} |
| 12 | `repair_code` ↻ | 是 | 重写 `game.js`（带报错），`repair_attempts++` |
| 13 | `replan_game_design` ↻ | 是 | 简化 `game_design`，`use_template_code=true` |
| 14 | `gameplay_qa` ◆ | 否 | `gameplay_qa_result`{passed, runtime_smoke_ok} |
| 15 | `gameplay_repair` ↻ | 否 | 放宽 `balance_config`（hazard 变慢 / +命） |
| 16 | `publish_artifact` | 否 | `game_id`、`version_id`、`manifest_url`、`/play/{id}` |
