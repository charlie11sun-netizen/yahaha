...
>
> 本文以当前代码为准；关键事实分别来自 `backend/app/agents/graph.py`、`backend/app/agents/pipeline.py`、`backend/app/agents/author_team.py`、`backend/app/services/task_dispatch.py`、`backend/app/services/packaging.py`、`backend/app/services/game_actions.py` 与 Docker Compose 配置。
...
| 可观测性 | structlog、Sentry、OpenTelemetry、AgentStep/AgentLog、AgentTraceEvent、LLMCall 账本 |
...
`backend/app/agents/graph.py` 当前定义 **24 个功能节点 + `done` / `failed`**。新建、修订和 Remix 共用同一张图，在 `memory_retrieval` 后按 `task_kind` 分支。
...
flowchart TB
    ...
    archetype --> assetProcess["asset_processing"]
    assetProcess --> design["game_design"]
    design --> content["content_plan"]
    content --> balance["balance_plan"]
    balance --> assetGen["asset_generation"]
    assetGen --> code["code_generation"]
...
2. **Agent 细节层**：Author Team 的 `team_started`、`contract_frozen`、`role_started/completed`、candidate merge/retry、integration evidence 等结构化事件写入日志 payload。
3. **高保真 trace 层**：可选 `AgentTraceEvent` 保存流式模型、工具调用、文件上下文和 patch 事件，按 task/step/run 检索。
4. **成本与基础设施层**：`LLMCall` 记录模型、prompt/completion/cached tokens、latency、retry、cost；structlog、Sentry 和 OpenTelemetry 关联 request/task/user 上下文。
...
│   │   ├── create/             # Create 工作区、SSE 和 Author Team 进度
...
│   │   ├── agents/             # LangGraph、Author Team、repair、validation、publish
│   │   ├── services/           # task/outbox、packaging、manifest、memory、业务动作
│   │   ├── tasks/              # Celery app、generation、outbox、memory jobs
...
| 2D Author Team 角色、预算、合并 | `backend/app/agents/author_team.py`、`author_runner.py` |
