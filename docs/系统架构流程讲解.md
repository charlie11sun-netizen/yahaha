...
> 基于当前架构图整理，重点说明 PlayForge 从用户创建游戏到 AI 生成、发布、播放的完整链路。
...
- **边界①　前端 → FastAPI（REST + JWT）**：浏览器永远只跟 FastAPI 通信并携带 JWT，鉴权、限流、发布等动作全部收口在这一层（`backend/app/api/routers/auth.py`、`backend/app/api/deps.py`）。
- **边界②　MinIO/S3 → 前端（iframe 远端加载 bundle）**：模型生成的游戏代码**从不在我们的服务器上执行**，而是作为静态产物上传对象存储，再由前端塞进 `<iframe sandbox="allow-scripts">` 运行，`manifest.json` 里把 `network/storage/cookies` 全部关闭。不可信代码被关在浏览器沙箱里，这是平台敢于直接运行 AI 生成代码的前提。
...
sequenceDiagram
    ...
    FE->>API: 轮询 GET /tasks/{id}
    FE->>API: GET /games/{id}/manifest → _url
    S3-->>FE: ② iframe 远端加载 bundle（sandbox）
```
...
FastAPI 是同步业务 API 层，负责登录鉴权、创建任务、查询任务、发布游戏、游戏列表、点赞、收藏、评论、排行榜等。前端和后端之间通过 REST + JWT 通信。

### Redis

Redis 在这里主要是 Celery 的任务队列 broker。FastAPI 收到生成请求后，不会自己跑 AI 生成，而是把任务投递到 Redis，等待 Celery Worker 消费。
...
GPT-5.5 是外部模型服务。Celery Worker 在需要模型创造能力的节点里调用它，例如生成游戏规格、玩法设计、代码和修复方案。

### 2.1 关键文件索引

| 环节 | 主要文件 |
| --- | --- |
| 登录 / JWT 鉴权 | `backend/app/api/routers/auth.py`、`backend/app/core/security.py`、`backend/app/api/deps.py` |
...
| 游戏 / manifest 接口 | `backend/app/api/routers/games.py`（`/games/{id}/manifest`，第 326 行起） |
...
用户在 Create 页输入游戏 prompt，上传参考素材，并选择 2D 或 3D。前端通过 REST + JWT 调用 FastAPI。JWT 用来证明当前请求属于哪个登录用户。
...
FastAPI 将任务投递给 Celery，本质上是在 Redis 中放入一条待执行任务。Redis 不保存业务最终状态，只负责让 Worker 能取到任务。

### 3.4 Worker 执行生成

Celery Worker 从 Redis 消费任务后，开始运行 LangGraph 生成流水线（`backend/app/agents/graph.py`）。架构图标的是“17 节点”，代码里实际是 **16 个功能节点 + `done` / `failed` 两个终止节点**。关键设计：**顶层图是写死的、确定性的**，保证安全检查、构建校验、上传这三道关永远不被跳过；“智能”只发生在节点内部。系统不是简单调用一次模型，而是通过多个节点逐步完成游戏生成。
...
- **规划**：`intent_spec` → `brief_expansion` → `mechanic_planner`——调模型。
- **路由**：`archetype_router`（选 2D/3D 原型：射击 / 跑酷 / 俯视收集 / 逻辑格 …）——纯 Python。
- **设计**：`asset_processing` → `game_design` → `content_plan` → `balance_plan`——仅 `game_design` 调模型。
- **生成 + 校验**：`code_generation`（调模型）→ `build_validation`（体积 / 安全扫描 / HTML 完整性，纯 Python）。
- **验收**：`gameplay_qa`（冒烟：有没有游戏循环 / 输入 / 重开）——纯 Python。
- **自愈**：`repair_code`、`replan_game_design`、`gameplay_repair`——见第 11 节。
- **发布**：`publish_artifact` → `done`——纯 Python。

整条流水线里**只有 `intent_spec` / `brief_expansion` / `mechanic_planner` / `game_design` / `code_generation` / `repair_code` / `replan_game_design` 这 7 个节点真正调用模型**，其余全是确定性 Python。另有 `use_real` 开关，关掉就全走启发式回退，整条链路能离线空跑（不连模型 / DB / S3），方便调试。
...
前端 Create 页不会直接连接 Celery，也不会直接读 Redis。它只会定时请求 FastAPI：
...
Worker 在生成流水线中的部分节点会调用 GPT-5.5，例如：
...
模型调用统一走 `backend/app/agents/llm.py` 的 `chat()`，底层是 OpenAI 兼容客户端（`base_url` / `api_key` / `model` 均来自配置），因此任何兼容 OpenAI Chat Completions 的服务都能接入。具体是哪 7 个节点调用模型，见第 3.4 节。

## 6. 产物上传流程

生成成功后，Worker 会把游戏产物上传到 MinIO/S3，常见结构如下：
...
其中 `manifest.json` 描述这个游戏的入口、文件列表、运行时类型、权限限制和版本信息。
...
Next.js Play 页面
-> FastAPI 获取游戏详情
-> FastAPI / S3 获取 manifest
-> iframe 从 MinIO/S3 远程加载 bundle
```
...
`replan_game_design` 会简化游戏设计并回到 `balance_plan` 重走后半段。**2D 会顺手把 `use_template_code` 置为 `True`**，用稳定模板兜底，保证这次一定能过校验；3D 没有 2D 模板可退，只简化设计、保持模型生成。
