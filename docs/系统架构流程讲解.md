...
整条流水线里**只有 `intent_spec` / `gameplay_planning` / `game_design` / `code_generation` / `repair_code` / `replan_game_design` 这 6 个节点真正调用模型**，其余全是确定性 Python。另有 `use_real` 开关，关掉就全走启发式回退，整条链路能离线空跑（不连模型 / DB / S3），方便调试。
...
模型调用统一走 `backend/app/agents/llm.py` 的 `chat()`，底层是 OpenAI 兼容客户端（`base_url` / `api_key` / `model` 均来自配置），因此任何兼容 OpenAI Chat Completions 的服务都能接入。具体是哪 6 个节点调用模型，见第 3.4 节。
