...
> 更新日期：2026-07-26。节点、路由和存储边界以 `backend/app/agents/graph.py`、`services/task_dispatch.py`、`services/runtime_urls.py` 为准。
...
MinIO/S3 是对象存储，保存用户上传素材和生成出来的游戏产物。当前 2D 生成链路同时保存 source project（`package.json`、`tsconfig.json`、`src/**`）与 sandbox 构建后的 `dist`；legacy/3D 产物仍可能包含 `index.html`、`style.css`、`game.js`、`three.min.js`。浏览器只通过 API 返回的 manifest/file token URL 读取发布文件。
...
| 创建任务 / 投递队列 | `backend/app/api/routers/tasks.py`（`POST /tasks`） |
| Celery 任务 / broker | `backend/app/tasks/generate.py`、`backend/app/tasks/celery_app.py` |
| 流水线编排 | `backend/app/agents/pipeline.py`（`run_generation`） |
| LangGraph 顶层图 | `backend/app/agents/graph.py`（节点与连边） |
| 节点实现 | `backend/app/agents/nodes.py`（门面）+ `planning_nodes.py` / `validation_nodes.py` / `nodes_common.py` |
| 设计契约与算术门禁 | `backend/app/generation/design_contract.py`（编译/冻结/账本校验） |
| 状态与回环上限 | `backend/app/agents/state.py`、`backend/app/generation/limits.py`（`MAX_REPAIR` 等） |
| 模型客户端 | `backend/app/llm/runtime.py`（OpenAI 兼容 `chat()`；`agents/llm.py` 为兼容别名） |
| 进度追踪 | `backend/app/agents/tracing.py`（`logged()` 装饰器） |
| 发布 / 打包 | `backend/app/services/packaging.py`（`publish_generated`） |
...
| 对象存储客户端 | `backend/app/storage/s3.py`（`put_object`） |
...
Celery Worker 从 Redis 消费任务后，开始运行 LangGraph 生成流水线（`backend/app/agents/graph.py`）。当前代码定义 **26 个功能节点 + `done` / `failed` 两个终止节点**。同一张图按 `task_kind` 分支支持新建、revision 和 remix；顶层顺序固定，模型只能在节点内部工作，安全、契约门禁、构建、玩法 QA 和发布不能被跳过。
...
- **契约**：`design_contract`（把设计确定性编译为 DesignContract 草稿）→ `contract_gate`（纯算术可行性校验后冻结契约、派生 sprite 需求等只读视图）——纯 Python，失败即任务失败。
- **素材**：`asset_processing` → `asset_generation`；消费冻结契约派生的素材需求，供应商按开关启用，支持逐 key 复用与增量重生成。
...
模型调用统一走 `backend/app/llm/`（runtime/provider/cache/accounting；旧 `agents/llm.py` 等路径保留为兼容别名）和 Author/Repair runner，底层是 OpenAI 兼容客户端或 Agents SDK（`base_url` / `api_key` / `model` 均来自配置），因此任何兼容 OpenAI 接口的服务都能接入。
...
用户上传素材
manifest.json
dist/index.html 与 dist/assets/*（2D 构建产物）
legacy index.html/style.css/game.js（兼容 bundle）
封面
Three.js 引擎文件（3D 自托管）
```
...
LangGraph 的优势体现在失败处理上。回环上限定义在 `backend/app/generation/limits.py`（由 `agents/state.py` 转发）：`MAX_REPAIR = 2`、`MAX_REPLAN = 1`、`MAX_GAMEPLAY_REPAIR = 2`；条件跳转函数 `should_continue_after_validation` / `should_continue_after_gameplay_qa` 实现在 `backend/app/agents/validation_nodes.py`，由 `graph.py` 接线。
...
gameplay_qa failed
-> gameplay_repair
-> 三选一：最小代码 patch 后重新构建
          / 调整平衡参数（回 balance_plan）
          / 数值重生成（回 code_generation）
```
