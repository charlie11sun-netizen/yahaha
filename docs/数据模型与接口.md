...
> 字段与流程对齐设计稿 `GameWeave.dc.html`（Home / Detail / Auth / Create / Play 五页面 + 5-Agent 流水线）。
> 更新日期：2026-07-01
...
```
逻辑地址 `oss://gameweave/games/<id>/<version>/manifest.json`；HTTP 访问 `https://<oss-endpoint>/games/<id>/<version>/…`。换真 S3/OSS 仅改 endpoint。
...
| GET | `/tasks/{id}` | 任务状态快照：`{status,current_step,steps[{agent,name,status,logs[]}],tokens,error,game?}`；也是 SSE 断线后的低频兜底 | 🔒 |
| GET | `/tasks/{id}/events` | SSE 任务事件流；先返回完整快照，Redis 收到失效信号后重新读取 PostgreSQL 并推送，终态后关闭 | 🔒 |
| GET | `/tasks` | 生成历史（加分） | 🔒 |
...
- **memory**：使用 PostgreSQL 范围过滤、BM25 与 embedding 余弦相似度双路排名，再通过 RRF 融合；embedding 暂存 JSON。后续数据量增大时迁移 `pgvector`，无需引入独立 ChromaDB。
