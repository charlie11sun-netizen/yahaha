...
def test_asset_prompt_uses_visual_requirements_for_contract_entities():
    plans = plan_game_assets(
        {
            "spec_execution_view": {"title": "Defense"},
                ...
                "entities": [
                    {
                        ...
                        "visual_requirements": ["moss-covered boulders, exposed roots and low shrubs"],
                    }
                ]
            },
        }
    )
    sheet = next(item for item in plans if item.key == "sheet")
    assert "moss-covered boulders" in sheet.prompt
