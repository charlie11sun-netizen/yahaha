...
def test_project_author_team_replays_planning_transcript_into_design_contract(
    ...
    # Stubbing every agent to None means no owner delivers, so the team
    # pauses for manual retry — after the contract phase has already
    # consumed the replayed transcript, which is what this test observes.
    with pytest.raises(AuthorTeamRetryRequired, match="integration was not started"):
...
def test_missing_owner_stops_before_integration_for_manual_retry(monkeypatch):
    """A missing required owner pauses the node instead of entering integration."""
    from app.core.errors import AuthorTeamRetryRequired
    ...
    def fake_execute(session, *, agent_name, **kwargs):
        ...
        if agent_name == "IntegrationAgent":
            ...
            raise AssertionError("integration must not start with a missing owner")
        _write_standard_candidate(session, agent_name)
        return _outcome(session)
    ...
    waiting = next(
        item for item in events if item.get("event") == "team_waiting_retry"
    )
    assert waiting["reason"] == "missing_required_roles"
    assert "RulesAndSimulationCoder" in waiting["missing_roles"]
    assert waiting["status"] == "failed"
    assert integration_started is False
...
def test_contract_repair_injects_genre_neutral_legibility_baseline():
    raw = json.loads(_contract_note())
    assert not any(
        item["id"] == "REQ-PRESENTATION-LEGIBILITY"
        for item in raw["acceptance"]
    )

    repaired, fixes = author_team._repair_contract(
        raw,
        {"genre": "rhythm"},
        {},
    )
    contract = author_team._freeze_contract(repaired, {"genre": "rhythm"}, {})

    requirement = next(
        item
        for item in contract.acceptance
        if item.id == "REQ-PRESENTATION-LEGIBILITY"
    )
    presentation = next(
        item
    ...
    assert "840x470" in requirement.requirement
    assert "16 effective CSS px" in requirement.requirement
    assert "14" in requirement.requirement
    assert "REQ-PRESENTATION-LEGIBILITY" in presentation.acceptance_ids
    assert any("injected mandatory acceptance" in fix for fix in fixes)
...
def test_author_prompts_use_actual_compact_embed_scale_arithmetic():
    for instructions in (
        author_team._DESIGN_CONTRACT_INSTRUCTIONS,
        author_team._PRESENTATION_INSTRUCTIONS,
        author_team._INTEGRATION_INSTRUCTIONS,
        author_team._PROJECT_AUTHOR_INSTRUCTIONS,
        author_team._PROJECT_REPAIR_INSTRUCTIONS,
    ):
        assert "cssScale=min(840/canvasWidth,470/canvasHeight,1)" in instructions
        assert "1280x720" in instructions
        assert "25px" in instructions
        assert "22px" in instructions
