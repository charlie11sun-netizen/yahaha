...
from app.observability.decision_trace import (
...
def _build_contract_observability_metadata(
    *,
    ...
    if gate:
        gate_issues = [
            str(issue)[:500]
            for issue in list(gate.get("issues") or [])[:8]
            if str(issue).strip()
        ]
        gate_summary = {
            "passed": gate_passed,
        ...
        metadata.update(
            {
                ...
                "contract_gate_issues": gate_issues,
...
def media_generation_span(
    *,
    ...
    """Span for one image/media provider call below the active stage span.

    第十二轮(2026-07-19)盲区:图像调用既无 span 也无 llm_calls 行,素材阶段
    38 分钟在 Opik 里只解释得了 ~7 分钟,三段 6-12 分钟的空窗全靠 docker
    日志拼图。线程池里调用时须由提交方 copy_context,否则 span 会脱离根
    trace 成为孤儿。
    """
    if not _apply_environment():
        yield None
    ...
    try:
        ...
        logger.warning("Opik media span unavailable: %s", exc)
        yield None
        return
    span_metadata = {
        "asset_key": key,
    ...
    with _fail_open_context(
        lambda: start_as_current_span(
            name=f"media.{key}",
            input={"asset_key": key},
            metadata={k: v for k, v in span_metadata.items() if v is not None},
            tags=["gameweave-media"],
        ),
        label=f"media span {key}",
    ) as span:
        yield span
