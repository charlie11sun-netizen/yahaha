...
_DEFAULT_ENTITY_VISUAL_REQUIREMENTS = (
...
def compile_design_contract(
    ...
    *,
    ...
    interaction_profiles = [
        dict(item)
        for item in (design.get("interaction_profiles") or [])[:24]
        if isinstance(item, Mapping)
    ] if isinstance(design.get("interaction_profiles"), list) else []
    required_profile_fields = (
        ...
        "clearance_or_timing_window",
        "feasibility",
    )
    profile_envelopes: dict[str, tuple[str, str]] = {}
    for index, profile in enumerate(interaction_profiles):
        missing = [
            field
        ...
        if missing:
            raise ContractCompileError(
                f"interaction profile {index + 1} missing fields: {', '.join(missing)}"
            )
        action = _text(profile.get("required_action")).lower()
        envelope_key = _canonical(
            {
                "visible_envelope": profile.get("visible_envelope"),
                "anchor": profile.get("anchor"),
                "interaction_envelope": profile.get("interaction_envelope"),
                "clearance_or_timing_window": profile.get(
                    "clearance_or_timing_window"
                ),
            }
        )
        previous = profile_envelopes.get(envelope_key)
        if previous and previous[0] != action:
            raise ContractCompileError(
                "mechanically distinct interaction profiles share an identical "
                f"spatial/timing envelope: {previous[1]} ({previous[0]}) and "
                f"{profile.get('id')} ({action})"
            )
        profile_envelopes[envelope_key] = (action, _text(profile.get("id")))
    ...
    for raw in rows:
        ...
        visual = [str(item) for item in _items(raw.get("visual_requirements")) if str(item).strip()]
        # The planning model historically emitted the human-readable artwork
        # brief under ``visual`` while the contract only retained
        # ``visual_requirements``.  Dropping that field left downstream asset
        # prompts with opaque entity ids (for example ``game_ab12...``), which
        # made semantic layout review unable to identify otherwise visible
        # sprites. Preserve the visual brief in the frozen contract without
        # changing the contract schema.
        visual_brief = _text(raw.get("visual") or raw.get("appearance") or raw.get("description"))
        if visual_brief and visual_brief not in visual:
            visual.insert(0, visual_brief)
        if not visual:
            visual = list(_DEFAULT_ENTITY_VISUAL_REQUIREMENTS)
    ...
    if interaction_profiles and not any(
        system.id == "interaction-fidelity" for system in systems
    ):
        systems.append(
            ContractSystem(
                id="interaction-fidelity",
                purpose=(
                    "resolve mechanically distinct spatial/timed interactions from shared "
                    "visible, collision, clearance, and timing profiles"
                ),
                inputs=["player_action", "subject_geometry", "target_geometry"],
                outputs=["interaction_success", "interaction_failure", "interaction_blocked"],
                runtime_consumers=["main.runtime", "gameplay.renderer", "gameplay.qa"],
                details={"interaction_profiles": interaction_profiles},
            )
        )
    if not systems:
    ...
    for index, profile in enumerate(interaction_profiles, start=1):
        profile_id = _text(
            profile.get("id") or profile.get("required_action") or f"interaction-{index}"
        )
        action = _text(profile.get("required_action") or profile_id)
        upsert_requirement(
            ContractRequirement(
                id=f"REQ-INTERACTION-{index:03d}",
                statement=(
                    f"Interaction {profile_id} ({action}) uses one profile for its visible anchor/envelope, "
                    "collision or interaction envelope, input semantics, activation/clearance/timing window, "
                    "resolution lifecycle, and feasibility arithmetic; spatial outcomes consume both subject "
                    "and target extents. Duration-bound actions define action duration or hold behavior, "
                    "earliest/latest useful activation, worst-case target occupancy, recovery, and cue timing."
                ),
                source_refs=[f"design:interaction_profiles[{index - 1}]"],
                resolved_as=["interaction-fidelity"],
                constraints=[
                    "different required actions use visibly distinct spatial/temporal envelopes",
                    "no center-line crossing plus semantic posture/kind result",
                    "imperative cues appear only while the action can remain active through resolution",
                    "current-frame action state is applied before contact resolution",
                ],
            )
        )
    upsert_requirement(
        ContractRequirement(
            ...
            statement=(
                ...
                "without collision pixels, coordinates, rule-layer/physics/state-machine prose, "
                "Probe/QA/debug labels, acceptance arithmetic, or developer instructions."
            ),
            source_refs=["policy:production-ui"],
            resolved_as=["presentation"],
            constraints=["published debug flag is false"],
        )
    )
    upsert_requirement(
        ContractRequirement(
            ...
            statement=(
                "Every resolved transient entity uses one stable id to remove or deactivate its renderer "
                "and collision/input body in the same logical tick, leaves future interaction queries, "
                "and emits Probe.despawn after the transition."
            ),
            source_refs=["policy:entity-lifecycle"],
            resolved_as=["main.runtime", "gameplay.qa"],
            constraints=[
                "score/state changes without the matching visible and physical transition are incomplete",
                "off-screen cleanup does not satisfy collection or destruction cleanup",
            ],
        )
    )
    ...
    for index, profile in enumerate(interaction_profiles, start=1):
        requirement_id = f"REQ-INTERACTION-{index:03d}"
        acceptance_id = f"AT-INTERACTION-{index:03d}"
        profile_id = _text(
            profile.get("id") or profile.get("required_action") or f"interaction-{index}"
        )
        action = _text(profile.get("required_action") or profile_id)
        acceptance = [item for item in acceptance if item.id != acceptance_id]
        acceptance.append(
            AcceptanceTest(
                id=acceptance_id,
                ...
                observable=(
                    f"{profile_id} is visually distinct, {action} is visibly feasible, and rendered "
                    "contact agrees with both success and failure outcomes."
                ),
                verification=(
                ...
                runtime_evidence=[
                    f"action:{action}",
                    f"window:{profile_id}:open",
                    f"outcome:{profile_id}:success",
                    f"outcome:{profile_id}:failure",
                ],
            )
        )
    acceptance = [
        ...
        if item.id
        not in {
            "AT-PRESENTATION-PRODUCTION-UI",
            "AT-INTEGRATION-ENTITY-LIFECYCLE",
        }
    ]
    acceptance.extend(
        [
            AcceptanceTest(
                id="AT-PRESENTATION-PRODUCTION-UI",
                requirement_ids=["REQ-PRESENTATION-PRODUCTION-UI"],
                owner="PresentationAndInteractionCoder",
                observable=(
                    "Start, active play, pause/choice, and terminal screens read like a finished game "
                    "rather than a test harness."
                ),
                verification=(
                    "Inspect every reachable runtime text state with the production debug flag false; "
                    "implementation diagnostics remain in Probe only."
                ),
                runtime_evidence=["production-ui:no-debug-copy"],
            ),
            AcceptanceTest(
                id="AT-INTEGRATION-ENTITY-LIFECYCLE",
                requirement_ids=["REQ-INTEGRATION-ENTITY-LIFECYCLE"],
                owner="IntegrationAgent",
                observable=(
                    "Each applicable resolved pickup, consumable, projectile, defeated actor, spent card, "
                    "destroyed object, or one-shot trigger disappears, transforms, or becomes inactive "
                    "exactly once and cannot be processed again."
                ),
                verification=(
                    "Correlate the stable entity id through the rules result, scene collection, renderer "
                    "plus physics/input transition, Probe.despawn, and a later tick that cannot process it."
                ),
                runtime_evidence=["despawn:resolved-entity"],
            ),
        ]
    )
...
def validate_contract(contract: DesignContract | Mapping[str, Any], *, require_frozen: bool = False) -> ContractGateResult:
    ...
    # Generic feasibility ledger.  Every game can express its own quantitative
    # claims as required capability vs available capability; the gate does not
    # need to know whether that means distance, jump reach, economy, board
    # capacity, reaction time, combat output, or another genre-specific unit.
    #
    # The legacy wave-combat fields remain supported for existing contracts.
    # Older non-combat plans sometimes emitted all-zero combat placeholders
    # together with an explicit N/A label.  Those placeholders are not failed
    # arithmetic; they mean the combat ledger was inapplicable.
    ledger_headroom: float | None = None
    opening_headroom: float | None = None
    feasibility_headroom: dict[str, float] = {}
    skipped_feasibility_ledgers = 0
    ...
    def _legacy_ledger_is_explicitly_inapplicable(
        ...
        legacy_keys = (
            ...
            "deliverable_damage",
            "first_wave_total_hp",
            "opening_deliverable_damage",
        )
        values = [_number(ledger.get(key)) for key in legacy_keys]
        if any(value is None or value != 0 for value in values):
            return False
        label = str(ledger.get("heaviest_wave") or "").strip().casefold()
        markers = (
            "n/a",
            "not applicable",
            "not-applicable",
        ...
        return any(label == marker or label.startswith(f"{marker}:") or label.startswith(f"{marker}：") for marker in markers)

    def _ledger_pair(ledger: Mapping[str, Any], demand_key: str, supply_key: str, margin: float, label: str, fix_hint: str):
        demand_raw, supply_raw = ledger.get(demand_key), ledger.get(supply_key)
        if demand_raw is None and supply_raw is None:
            return None
        demand, supply = _number(demand_raw), _number(supply_raw)
        if demand is None or supply is None:
            issues.append(f"win_feasibility_ledger must provide numeric {demand_key} and {supply_key}")
            return None
        if demand <= 0 or supply <= 0:
            issues.append(
                f"win_feasibility_ledger numbers must be positive ({demand_key}={demand}, {supply_key}={supply})"
            )
            return None
        if supply < margin * demand:
            issues.append(
                f"win_feasibility_ledger shows insufficient {label} headroom: "
                f"{supply_key} {supply:g} < {margin} x {demand_key} {demand:g} — {fix_hint}"
            )
        return round(supply / demand - 1.0, 4)

    for system in model.systems:
        ledger = system.details.get("win_feasibility_ledger")
        if not isinstance(ledger, Mapping):
            continue
        if _legacy_ledger_is_explicitly_inapplicable(ledger):
            skipped_feasibility_ledgers += 1
            continue
        if "checks" in ledger:
            raw_checks = ledger.get("checks")
            if not isinstance(raw_checks, (list, tuple)) or not raw_checks:
                issues.append("win_feasibility_ledger checks must be a non-empty list")
                continue
            seen_check_ids: set[str] = set()
            for index, raw_check in enumerate(raw_checks):
                if not isinstance(raw_check, Mapping):
                    issues.append(
                        f"win_feasibility_ledger check {index + 1} must be an object"
                    )
                    continue
                check_id = str(raw_check.get("id") or "").strip()
                if not check_id:
                    issues.append(
                        f"win_feasibility_ledger check {index + 1} requires an id"
                    )
                    continue
                if check_id in seen_check_ids:
                    issues.append(
                        f"duplicate win_feasibility_ledger check id: {check_id}"
                    )
                    continue
                seen_check_ids.add(check_id)
                required = _number(
                    raw_check.get("required", raw_check.get("demand"))
                )
                available = _number(
                    raw_check.get("available", raw_check.get("supply"))
                )
                minimum_ratio = _number(raw_check.get("minimum_ratio"))
                if required is None or available is None:
                    issues.append(
                        f"win_feasibility_ledger check {check_id} must provide "
                        "numeric required and available"
                    )
                    continue
                if required <= 0 or available <= 0:
                    issues.append(
                        f"win_feasibility_ledger check {check_id} numbers must "
                        f"be positive (required={required}, available={available})"
                    )
                    continue
                if minimum_ratio is None or minimum_ratio < 1:
                    issues.append(
                        f"win_feasibility_ledger check {check_id} requires a "
                        "numeric minimum_ratio >= 1"
                    )
                    continue
                if available < minimum_ratio * required:
                    unit = str(raw_check.get("unit") or "units").strip()
                    issues.append(
                        f"win_feasibility_ledger check {check_id} has "
                        f"insufficient headroom: available {available:g} {unit} "
                        f"< {minimum_ratio:g} x required {required:g} {unit}"
                    )
                feasibility_headroom[check_id] = round(
                    available / required - 1.0,
                    4,
                )
            continue
        headroom = _ledger_pair(
            ledger,
            "threat_effective_hp",
            "deliverable_damage",
            1.3,
            "capacity",
            "shrink the heaviest wave/phase composition, lower its HP or support stacking, "
            "or raise the attainable endgame output in the design content",
        )
        if headroom is not None:
            ledger_headroom = headroom
        headroom = _ledger_pair(
            ledger,
            "first_wave_total_hp",
            "opening_deliverable_damage",
            1.2,
            "opening",
            "weaken wave 1 or strengthen/cheapen the tutorial opening — chord-based output of the "
            "opening loadout must clear the first wave without leaks",
        )
        if headroom is not None:
            opening_headroom = headroom
    metrics = {
        ...
        **({"capacity_headroom": ledger_headroom} if ledger_headroom is not None else {}),
        **({"opening_headroom": opening_headroom} if opening_headroom is not None else {}),
        **(
            {
                "feasibility_check_count": len(feasibility_headroom),
                "feasibility_headroom": feasibility_headroom,
            }
            if feasibility_headroom
            else {}
        ),
        **(
            {"skipped_feasibility_ledger_count": skipped_feasibility_ledgers}
            if skipped_feasibility_ledgers
            else {}
        ),
    }
...
def contract_to_design_payload(contract: DesignContract | Mapping[str, Any]) -> dict[str, Any]:
    ...
    for entity in model.entities:
        visual_brief = "; ".join(
            item
            for item in entity.visual_requirements
            if item not in _DEFAULT_ENTITY_VISUAL_REQUIREMENTS
        )
        row = {"id": entity.id, "name": entity.id, "role": entity.role, "footprint": entity.footprint, "interactions": entity.interactions, "states": [state.id for state in entity.states], "runtime_consumers": entity.runtime_consumers, "visual_requirements": entity.visual_requirements, "visual": visual_brief}
    ...
    for entity in model.entities:
        for state in entity.states:
            ...
            demands.append(SpriteDemand(
                ...
                metadata={
                    "role": entity.role,
                    "visual_requirements": entity.visual_requirements,
                    "description": "; ".join(
                        item
                        for item in entity.visual_requirements
                        if item not in _DEFAULT_ENTITY_VISUAL_REQUIREMENTS
